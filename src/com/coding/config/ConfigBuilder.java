...
import com.coding.config.util.Util;
 ...
 * COMPLETE: if the execution succeeded without any errors. IN_PROGRESS: when
 * starting the processing of the config file.
 *
 */
public class ConfigBuilder implements Runnable {
	...
	public void run() {
		...
		/*
		 * Getting a singleton instance of the Config class
		 */
		...
		try {
			...
			while (scanner.hasNextLine()) {
				...
				if (line.startsWith(Constants.OPENING_BRACKET, 0)
					 ...
					 * we will put this map inthe configHashMap.
					 */
					if (map != null) {
					...
					groupName = groupName.replace(Constants.CLOSING_BRACKET,
							Constants.EMPTY_STRING);
				...
				if (keyValuePair.length != 2) {
					...
					throw new IllegalArgumentException(
							"proper key value pair not found in config file for group: "
									+ groupName);
				}
				...
				if (key.indexOf(Constants.LESS_THAN) > -1) {
					key = key.replace(Constants.GREATER_THAN,
							Constants.EMPTY_STRING);
			...
			LOG.info("Parsing of config file complete from config file location: "
